...
Let's explore the built in mtcars data frame before we get to using the TCGA dataframe to learn about this crucial data type.
...
mtcars[7, 4] # horsepower of Duster 360
mtcars[16, 4] # horsepower of Lincoln Continental
mtcars[31, 4] # horsepower of Maserati Bora
...
```{r}
write.csv(clinic, "/home1/kyolsen/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/kyolsen/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
# 4. How old was the 1st patient at their last follow up (barcode: TCGA-3C-AAAU)? HINT: you need to look at data from two columns to answer this: (20211+3767)/365 = 65
...
plot(
  ...
  clinic$lymph_nodes_examined_count,
  
  main = "Scatter Plot of Last Contact Days To vs. Lymph Nodes Examined Count",
  xlab = "Days to Last Contact",
  ylab = "Lymph Nodes Examined",
  col = "green"
)
```
...
# create plot here
boxplot(
  clinic$age_at_diagnosis / 365 ~ clinic$vital_status,  # Convert age to years
  main = "Boxplot of Age at Diagnosis by Vital Status",
  xlab = "Vital Status",
...
4) Using your under30_over60_mask, create a new dataframe called no_middle_clinic which contains all the clinical data for patients under the age of 35 or over 60 at diagnosis
...
# 2. What does the x axis represent?: time since diagnosis

# 3. What age group appears to have the greatest rate of survival?: middle aged

# 4. What age group appears to have the worst rate of survival?: young

# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?: 0.30 or 30%

# 6. What is the p-value? Is it statistically significant and what does that tell us about the relationship between age and breast cancer survival?: p = 0.2, which is not statistically significant. This means there is not strong evidence for a relatioship between age and breast cancer survival. In other words, we cannot reject the null hypothesis from this data.
